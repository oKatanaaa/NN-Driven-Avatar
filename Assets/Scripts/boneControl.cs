...
public class boneControl : MonoBehaviour
{
    ...
    {   
        Vector3 some = new Vector3(1f, 0.2f, 0.2f);
        ...
        for (int i = 0; i < k_NumSkeletonJoints; ++i)
        {
            ...
            if (bone != null)
            {
                //bone.transform.position = bone.transform.localPosition+some;
                bone.transform.localRotation *= Quaternion.Euler(10, 10, 10);
            }
        }
        m_SkeletonRoot.position += some;
    }
    ...
    {
        JointIndices val;
        if (System.Enum.TryParse(jointName, out val))
        {
            return (int)val;
        }
        return -1;
